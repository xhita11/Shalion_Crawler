...
class Products(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for product in data['keywords']:
            urls.append(url_1 + product.replace(' ','+') + url_2)
        ...
        '''
            for url in urls:
            print('\n\n\n\n')
            # scrapy.Request(url=url, callback=self.parse_product)
            yield scrapy.Request(url=url, callback=self.parse_product, meta={'Nom':'Hola'})
       '''     
    ...
    def parse_product(self, response):
        ...
        for result in jsonresponse['data']['search']['products']:
            ...
            price_amount = result['price']['current_retail']
            ...
            #print(result['item']['product_description']['title'])
            #results.append(result['item']['product_description']['title'])
